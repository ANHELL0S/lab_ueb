...
import { KEY_REDIS_ROLE, KEY_REDIS_USER, TIME_KEY_VALID } from '../const/redis_keys.const.js'
	...
	async getAllRoleUsers(req, res) {
		...
		try {
			...
			if (cachedData) {
				...
				return sendResponse(res, 200, 'Roles de usuarios obtenidos exitosamente.', parsedData)
			}

			const { count, rows } = await user_role_main_Schema.findAndCountAll({
				include: [
					{
			...
			if (rows.length === 0) {
				return sendResponse(res, 200, 'No hay más roles de usuarios disponibles.', {
					totalRecords: count,
			...
			const responseData = {
				...
				recordsPerPage: limit,
				data: {
					roles_user: rows.map(row => ({
						id_user_role_intermediate: row.id_user_role_intermediate,
			...
			return sendResponse(res, 200, 'Roles de usuarios obtenidos exitosamente.', responseData)
		} catch (error) {
			await logEvent(
				'error',
				'Error al obtener los roles de usuarios.',
